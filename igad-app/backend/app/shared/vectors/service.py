...
class VectorEmbeddingsService:
        ...
        try:
            ...
            for index in ["reference-proposals-index", "existing-work-index"]:
                ...
                response = self.s3vectors.list_vectors(
                    vectorBucketName=self.bucket_name, indexName=index
                )
                ...
                if keys:
                    self.s3vectors.delete_vectors(
                        vectorBucketName=self.bucket_name, indexName=index, keys=keys
                    )
    ...
    def delete_vectors_by_document_name(
        self, document_name: str, index_name: str
    ) -> bool:
        """
        ...
        try:
            ...
            if keys_to_delete:
                print(
                    f"Deleting {len(keys_to_delete)} vectors for document {document_name}"
                )
                self.s3vectors.delete_vectors(
                    vectorBucketName=self.bucket_name,
            ...
            for doc_name, doc_vectors in docs_by_name.items():
                ...
                try:
                    ...
                    s3_response = self.s3.get_object(
                        Bucket=self.documents_bucket, Key=s3_key
                    )
                    ...
                    print(
                        f"✅ Reconstructed {doc_name} ({ext.upper()}, {len(doc_vectors)} chunks)"
                    )
    ...
    def search_and_reconstruct_proposals(
        ...
        try:
            ...
            sorted_docs = sorted(
                docs_by_name.items(), key=lambda x: x[1]["avg_distance"]
            )[:top_k]
            ...
            for doc_name, doc_info in sorted_docs:
                try:
                    ...
                    print(
                        f"   📄 Reconstructing: {doc_name} (similarity: {1 - doc_info['avg_distance']:.2%})"
                    )
                    ...
                    print(
                        f"      ✅ Reconstructed ({len(full_text)} chars, {len(doc_info['chunks'])} chunks)"
                    )
