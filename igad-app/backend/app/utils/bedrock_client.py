...
class BedrockClient:
    ...
    def improve_content(
        ...
        return self.generate_content(
            prompt=prompt, system_prompt=system_prompt, max_tokens=2000, temperature=0.5
        )

    def generate_executive_summary(
        self, proposal_sections: Dict[str, str], max_tokens: int = 800
    ) -> Dict[str, Any]:
        ...
        sections_text = "\n\n".join(
            [f"**{title}:**\n{content}" for title, content in proposal_sections.items()]
        )
