...
- **Bedrock `ResourceNotFoundException: ... marked by provider as Legacy`**: AWS Bedrock revokes access to legacy Claude models after 30 days of inactivity. Update the `model` field in the affected `app/tools/**/config.py` to a current cross-region inference ID (e.g. `us.anthropic.claude-haiku-4-5-20251001-v1:0` for Haiku, `us.anthropic.claude-sonnet-4-5-20250929-v1:0` for Sonnet). Search for occurrences with `grep -rn "anthropic.claude" backend/app`.
- **`sam build` hangs at 0% CPU on macOS**: Inspect `igad-app/.aws-sam/build/` for duplicate folders with numeric suffixes like `ApiFunction 2`, `ApiFunction 3` (created by iCloud Drive / Finder when the Desktop folder is synced and a previous build was interrupted). SAM walks them recursively and stalls. Fix: kill the hung `sam build` process, remove every duplicate folder under `.aws-sam/build/`, then re-run the deploy. APFS deletes can take minutes per folder when the disk is >90% full — check with `df -h /` and free space if cleanup is unusually slow.
