...
class CognitoUserManagementService:
    def __init__(self, user_pool_id: str, client_id: str, region: str = "us-east-1"):
        ...
        print(f"Initialized Cognito client with profile IBD-DEV, region: {region}")  # Debug
        ...
        try:
            ...
            for user in response.get('Users', []):
                ...
                try:
                    ...
                    print(f"Error getting groups for user {user_data['username']}: {group_error}")
                    user_data['groups'] = []
    ...
    def reset_user_password(self, username: str, temporary_password: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for group in response.get('Groups', []):
                groups.append({
                    ...
                    'creation_date': group['CreationDate'].isoformat() if group.get('CreationDate') else None,
                    'last_modified_date': group['LastModifiedDate'].isoformat() if group.get('LastModifiedDate') else None
                })
    ...
    def create_group(self, group_name: str, description: str = "", precedence: Optional[int] = None) -> Dict[str, Any]:
        """Create a new group"""
        try:
            params = {
                'UserPoolId': self.user_pool_id,
            ...
            return {
                ...
                'group': {
                    ...
                    'creation_date': response['Group']['CreationDate'].isoformat() if response['Group'].get('CreationDate') else None,
                    'last_modified_date': response['Group']['LastModifiedDate'].isoformat() if response['Group'].get('LastModifiedDate') else None
                },
    ...
    def _format_user_data(self, user_data: Dict) -> Dict[str, Any]:
        ...
        return {
            ...
            'last_modified_date': user_data.get('UserLastModifiedDate').isoformat() if user_data.get('UserLastModifiedDate') else None,
